...
**Correction, from actually doing the deferred step later:** the Cloudflare-side Public Hostname route (`wallabag.cluster.download` → `http://10.100.102.10:80`) was sufficient to get traffic through the tunnel to nginx, confirmed live via a request forced through Cloudflare's real edge (bypassing dnsmasq's local split-horizon so the test actually exercised the tunnel). But the assumption that nothing in this repo would need to change was wrong: `k8s/apps/wallabag/ingress.yaml` only had a rule for `wallabag.pi-cluster.lan`, and Traefik's ingress provider matches the Host header by exact string against `spec.rules[].host` — no `.lan`/`.cluster.download` equivalence. A request arriving with `Host: wallabag.cluster.download` matched no rule and hit Traefik's default 404 until a second `rules[]` entry was added for that host. `SYMFONY__ENV__DOMAIN_NAME` is left pinned to the `.lan` value (see the Configuration notes above) — Wallabag's own generated absolute links still read the LAN host regardless of which hostname you browsed in on; only browsing is affected by this fix, not link generation.
