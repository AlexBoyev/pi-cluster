...
- **OCR languages**: `PAPERLESS_OCR_LANGUAGES=heb+eng` (Tesseract's `+` syntax installs and uses both) — confirmed the Docker image's `install_languages` entrypoint logic installs `tesseract-ocr-heb` at container startup when named in this variable; **not** assumed to work without verification — see "Verified live" below for the actual OCR-accuracy check the brief calls the single most important step in this phase.
