...
**Also observed while applying the fix: individual Traefik pods got stuck `Terminating` past their grace period more than once during the rollout** (first pi-node1's old pod, then pi-node3's) — required `--grace-period=0 --force` to clear. This happened on nodes with no port conflict, so it's not fully explained by the pi-node1 root cause above; combined with all 4 pods' pre-existing high restart counts (8-12 each, including the 3 nodes that never had a port conflict), there may be a second, still-unidentified issue with how Traefik terminates on this cluster. Worth keeping an eye on via the new `PodCrashLooping`/`PodNotReady` alerts rather than considered closed. **Update, 2026-09-07**: that stuck-`Terminating` symptom is now explained — it's a `D`-state (uninterruptible disk-wait) kernel process, which no signal including `SIGKILL` can clear. See the cold-boot `uas`-driver incident below.

**Follow-up done, 2026-09-07**: `k8s/traefik/traefik.yaml` moved to `k8s/apps/traefik.yaml` — it's GitOps-managed now, no more manual `kubectl apply` special case. See the roadmap Phase 66 entry.

## Staggered k3s-agent boot delay, to avoid a simultaneous cold-boot I/O storm

**Incident, 2026-09-07**: after a full power cycle of all 4 Pis (one power source, no way to stagger physically), every household service (Wallabag, Vikunja, Paperless) and the dashboard itself returned 502 for 18+ minutes. Root cause traced to pi-node2/3/4 all rebooting in the same few seconds: each node's containerd tried to start every DaemonSet pod scheduled to it (Traefik, node-exporter, promtail) at once, saturating that node's own local disk (a Kingston A400 SSD over USB — no DRAM cache, poor sustained random I/O) badly enough that Traefik's container was still stuck inside its own entrypoint's `traefik --help` self-check, blocked on `blk_mq_get_tag`, 18 minutes after starting — never reaching the point of binding port 80. `kubectl get pods` reported it `Running`/`1/1 Ready` the entire time (no probes existed — see the Traefik manifest change below), so nothing in the platform surfaced this as unhealthy; it had to be diagnosed by hand via `crictl inspect`/`nsenter` on each worker.

Since the physical power-on can't be staggered, the fix is software: a per-node boot delay before `k3s-agent` starts, via a systemd drop-in (`/etc/systemd/system/k3s-agent.service.d/boot-delay.conf`, `ExecStartPre=/bin/sleep N`) applied by the `k3s_agent` Ansible role, driven by a `boot_delay_seconds` inventory var (`ansible/inventory/hosts.ini`: pi-node2=0, pi-node3=60, pi-node4=120). This doesn't make any single node's own local disk faster, but it spreads each node's own pod-start burst across ~2 minutes instead of all landing in the same 30-second window, and — since node registration and scheduling round-trip through the single control-plane API server/sqlite datastore on pi-node1 — stops all 3 workers from hammering that shared choke point at once too, which was likely also why `dnsmasq` on pi-node1 was observed restart-looping during the same window (not fixed here; flagged as a separate follow-up).

Rejected: staggering physical power-on (no hardware supports it on this setup); a shared/network storage backend for the workers (out of scope, a much bigger change than an outage this size warrants); throttling containerd/kubelet image-pull concurrency (irrelevant here — the images were already present locally, the bottleneck was container *start*, not pull).

**Correction, same incident, found ~1 hour later: the stagger above was not the actual fix.** After deploying it and rebooting all 3 workers to test, all 3 Traefik pods hung again with the *identical* symptom — `traefik traefik --help` stuck in kernel disk-wait — even though the stagger meant they were no longer starting at the same instant (pi-node4 correctly waited its full 120s and then hung anyway, alone, at a load average around 2.5). That ruled out cross-node contention as the root cause: this is a **per-node** hardware issue, not a shared-resource one.

Root cause, confirmed via `dmesg`: all 4 nodes' Kingston SA400S37 SSDs sit behind an identical ASMedia ASM235CM USB-SATA bridge (`174c:235c`), and Linux's `uas` driver for that specific bridge is known to hang on the first burst of small reads after a cold boot on Raspberry Pi — a documented class of Pi + USB-SSD problem, not specific to this cluster. Fixed with a kernel boot parameter forcing the older `usb-storage` driver instead (`usb-storage.quirks=174c:235c:u` in `/boot/firmware/cmdline.txt` — **not** `/boot/cmdline.txt`, which is a placeholder on this OS version; the `common` role's existing cgroups task had been silently targeting that same wrong path since some earlier Raspberry Pi OS upgrade, caught and fixed alongside this). Verified live: pi-node2 went from a 40+ minute hang to Traefik binding port 80 in under 30 seconds after the fix, reproduced the same way on pi-node3 and pi-node4. Applied to all 4 nodes (`ansible/roles/common/tasks/main.yml`) — pi-node1 has the same hardware and was staged with the same fix, even though it hadn't shown the symptom, since a future unlucky cold boot hanging the control plane this way would be worse than any worker outage.

The boot-delay stagger (above) is kept regardless — it's still correct defense in depth against genuine cross-node contention on any slower future storage, and cost nothing to leave in place. But it was not what fixed this incident; the USB quirk was.
...
# ADR: Household Services — Paperless-ngx (third of four)

Paperless-ngx (household document archive) is the third household service. It reuses the established pattern in full — namespace, `local-path` PVC, database inside the platform Postgres, nginx wildcard/Traefik routing for the LAN/public hostnames, out-of-band Secret, pinned arm64 images — pinned to **pi-node2** specifically, alone, per the brief: OCR spikes and pins a CPU core, and pi-node2 currently runs nothing but the platform DaemonSets (confirmed live: `kubectl top node pi-node2` → 113m CPU/2%, 968Mi memory/12% used at cgroup accounting; `free -h` on the node itself → 7.6Gi total, 518Mi used, 6.2Gi free; `df -h /` → 220G total, 5.2G used, 206G available). This is explicitly a ten-year archive, not a demo — durability and correct Hebrew+English retrieval matter more than raw throughput, and the decisions below are weighted accordingly.

Four real decisions, proposed as an ADR and confirmed before any manifest was written, per the brief's own requirement.

## D1 — Office document support: Tika + Gotenberg, in

`ghcr.io/paperless-ngx/paperless-ngx:2.20.15` — confirmed via direct GHCR manifest query (not a docs page) to carry a real `linux/arm64` manifest (`sha256:123a6ada...`, matching `amd64`'s `sha256:835974fc...`). Tika (`apache/tika:4.0.0`) and Gotenberg (`gotenberg/gotenberg:8.36.0`) both independently confirmed `linux/arm64` the same way.

Given the household's paperwork explicitly includes school and municipal documents (commonly `.docx`), and pi-node2 has ~6.2Gi genuinely free with nothing else scheduled there, both are enabled. Honest gap, stated rather than papered over: no verified idle-RAM benchmark exists for either image on arm64/Pi hardware specifically — Paperless's own docs only give a combined "4GB for the full stack" figure (app + Postgres + Redis + Tika + Gotenberg together), not a per-container number, and both are known-heavy process classes (JVM heap for Tika, a headless Chromium instance for Gotenberg) rather than lightweight converters. Both are deployed with real container memory limits (512Mi each) set deliberately conservative, and the **real** figure is committed to this document (see "Verified live" below) after being measured against an actual Hebrew/Word document during rollout — not invented here.

## D2 — Ingestion path: Samba share (primary, authenticated, LAN-only) + web upload/mobile app (secondary)

Rejected web-upload-only: it technically satisfies nothing new over what Paperless already ships — it's still manual action per document, and the brief's own goal was "without me copying them by hand every time." Rejected NFS: already rejected cluster-wide in the Wallabag ADR (D1), nothing here changes that reasoning. Rejected Syncthing: real per-device cryptographic auth, but every device needs the app installed and paired individually, and it doesn't integrate with a scanner's "scan to network folder" feature the way SMB does — more setup friction than SMB for a two-person household with a physical scanner/printer in the loop. Email/IMAP ingestion is explicitly deferred by the brief, designed for but left disabled.

**Security story, since this volume holds medical and financial documents**: a dedicated `ghcr.io/servercontainers/samba:smbd-only-a3.18.2-s4.18.5-r0` Deployment (confirmed `linux/arm64`) in the `paperless` namespace, mounting the same PVC's `consume/` subPath, with its **own Samba user/password** — a credential entirely separate from pi-cluster SSO (SMB doesn't speak HTTP/cookies, so it was never going to participate in the `pi_sso` gate anyway). Exposed via `hostNetwork: true` on pi-node2 rather than a K8s `NodePort` Service — SMB clients (Windows Explorer especially) expect literal port 445, not an arbitrary port in the NodePort range (30000–32767), and `hostNetwork` is the same technique this cluster's own `dnsmasq`/`cloudflared` Compose services already use (`network_mode: host`) for the identical reason. Never through nginx or the Cloudflare Tunnel either way — `cloudflared` only ever forwards to `nginx:80`, so there is no path from the public internet to this share at all, by construction, not by firewall rule. iOS Files app, Android file managers, Windows/macOS Finder, and most networked scanners all speak SMB natively.

## D3 — Search index and rebuild cost: measured live, not estimated

Confirmed (Paperless 2.20.x): the full-text index (Whoosh) lives under the `data/` PVC subPath, alongside the classifier model (`document_classifier.pickle`) and Celery/task-queue state — a **different** subPath from `media/`, which holds the actual document files; losing either independently is bad in different ways, so both are backed by the same PVC (not portable across nodes, same tradeoff already accepted for Wallabag/Vikunja) rather than treated as disposable.

No invented benchmark: the one real data point found (a 491,548-document deployment with a 204GB index) establishes that rebuild scales with corpus size and is disk-I/O-heavy, not that it's fast on Pi-class ARM hardware specifically. `document_index reindex` will be timed for real against this household's actual document count during rollout and after the initial bulk import, and both numbers recorded here — if either lands in "hours" territory, the backup/upgrade runbook in `docs/operations.md` reflects that explicitly rather than assuming a quick rebuild.

One forward-looking note, not a blocker: upstream has an open PR replacing Whoosh with `tantivy` (Rust-based) as the search backend, not shipped in 2.20.x. Worth re-checking reindex characteristics on the next version upgrade rather than assuming they're unchanged.

## D4 — SSO: `PAPERLESS_ENABLE_HTTP_REMOTE_USER`, not a stored-credential bridge

Deliberately **not** a repeat of Wallabag/Vikunja's auto-login bridge pattern. Both of those store a real password server-side because neither app has a clean way to accept pre-verified identity from a reverse proxy. Paperless does: `PAPERLESS_ENABLE_HTTP_REMOTE_USER` is its own documented mechanism for exactly this — nginx sets a `Remote-User: <username>` header after `auth_request` succeeds, and Paperless trusts it and logs the user in, with no password of Paperless's own ever touching this repo's `.env`. This is a real reduction in the "credential doubling" tradeoff the Vikunja ADR flagged as a cost, not a new pattern needing its own separate justification.

Two things make this safe rather than merely convenient:

1. **Header-injection risk, addressed structurally.** nginx's `proxy_set_header Remote-User $verified_user` *replaces* any client-supplied value unconditionally on every proxied request — it does not append — so a request arriving from the internet with a forged `Remote-User` header cannot get through, as long as this is set on every request to Paperless's hostname. This needed a **dedicated** nginx `server` block for `paperless.pi-cluster.lan`/`paperless.cluster.download` — not the shared household-services wildcard fallback, since only Paperless needs the header captured and injected. `GET /api/v1/auth/verify` was extended to return the authenticated username in an `X-Pi-User` response header on success; the new block's `auth_request_set $pi_user $upstream_http_x_pi_user;` captures it before `proxy_set_header Remote-User $pi_user;` to Traefik. This is the second time the Wallabag-era assumption ("the wildcard fallback handles every household service identically") didn't hold — Vikunja needed a gate *exemption* for CalDAV; Paperless needs a *dedicated* block for header injection. Reusable as a pattern for Firefly if it ever supports the same mechanism.
2. **API access stays on its own auth, unaffected.** `PAPERLESS_ENABLE_HTTP_REMOTE_USER_API` is a **separate** setting, left `false` — the mobile app and any future companion service (the out-of-scope AI-enrichment service the brief anticipates) keep authenticating via Paperless's own per-user API token, exactly satisfying the requirement that an interactive login gate must never break a non-interactive client, the same trap CalDAV hit for Vikunja.

## Configuration notes, verified rather than assumed

- **OCR languages**: `PAPERLESS_OCR_LANGUAGE=heb+eng+rus` (Tesseract's `+` syntax combines all three; order is priority, Hebrew first) — corrected from an initial `heb+eng`-only assumption after explicit user input that Russian is a real, regular language in this household's document mix too, not just Hebrew and English. `PAPERLESS_OCR_LANGUAGES=heb rus` installs both extra apt packages at container startup (`eng` is already built into the base image) — confirmed the Docker image's `install_languages` entrypoint logic does this when named in this variable; **not** assumed to work without verification — see "Verified live" below for the actual OCR-accuracy check the brief calls the single most important step in this phase.
- **Date parsing**: `PAPERLESS_DATE_ORDER=DMY`, per the brief — Israeli documents default-parsed with the wrong order produce wrong dates, which are worse than no date at all.
- **Redis**: reuses the platform's existing Redis instance (`10.100.102.10:6379`, already running in Docker Compose, already reachable from K8s pods over the LAN — same reasoning as Postgres reuse) on database index `1` (the backend's own cache/locks use index `0` — confirmed via `settings.redis_url`) instead of a new Redis pod. No new stateful component for a queue Paperless barely uses at this household's scale.
- **Barcode/ASN splitting**: not enabled, per the brief — the config surface is left in its default (off) state; nothing to design around since Paperless's own bulk-consume path doesn't require it either.
- **Registration/accounts**: Paperless has no self-registration to disable (unlike Wallabag/Vikunja) — accounts are admin-created via `manage.py createsuperuser`/the admin UI from the start, no extra closing step needed.
...
## Discovered while deploying this: `k8s/apps/` was never actually recursed — every household service had been GitOps-managed in name only

Applying Firefly's manifests (all under `k8s/apps/firefly/`, same as every prior service) and waiting for ArgoCD's automated sync produced nothing — no resources, no error, `kubectl get application pi-cluster -n argocd -o jsonpath='{.status.resources[*]}'` showed zero Wallabag/Vikunja/Paperless resources or images either, despite all three running fine live. Root cause: the live Application's `spec.source` has no `directory.recurse: true`, and ArgoCD's plain-directory source type defaults to non-recursive — it was only ever applying the root-level files directly in `k8s/apps/` (`node-exporter.yaml`, `promtail.yaml`, `kube-state-metrics.yaml`, `traefik.yaml` as of its GitOps-migration this same session, `namespace.yaml`, `sample-nginx.yaml`), never descending into `wallabag/`, `vikunja/`, or `paperless/` at all.

Every one of those three services was, in fact, running the whole time purely because their own README's "or apply directly to skip the ~3 minute wait" `kubectl apply -f k8s/apps/<name>/` fallback was what actually got used during each rollout — not the automated sync every architecture doc and README described as the normal path. **A push to git alone would never have updated any household service** since the day each was first deployed; this had been silently true for the entire lifetime of Wallabag, Vikunja, and Paperless, discovered only because Firefly's first-ever deploy happened to depend on the automated path actually working.

**Also discovered while fixing this**: neither `terraform/argocd.tf` (declares a resource named `pi-cluster-apps` targeting namespace `pi-cluster`) nor `ansible/playbooks/argocd.yml` (only ever runs a one-time `kubectl apply -f k8s/apps/`, never creates a watching `Application` resource at all) matches the actual live Application (`pi-cluster`, namespace `pi-apps`). Its real origin is unknown — created some other way, outside both IaC paths, at some point in the project's history before either file was written to match it. Corrected `terraform/argocd.tf`'s name/namespace to match reality and added `directory.recurse = true` there, in addition to patching the live resource directly (`kubectl patch application pi-cluster -n argocd --type merge -p '{"spec":{"source":{"directory":{"recurse":true}}}}'`) so this didn't wait on a full Terraform apply. Verified live: Wallabag/Vikunja/Paperless pods were undisturbed by the patch (same pod names/ages, zero new restarts) and Firefly's manifests were picked up and applied within seconds of the refresh.
...
## Incident, 2026-09-08 — `PodCrashLooping`/`PodNotReady` on `firefly-cron` was a symptom of pi-node3 undervoltage, not a Firefly bug

Alerts fired on `firefly-cron`'s 5 restarts. Its actual logs showed `curl` exit code 7 (connection refused) on every failed attempt, and the job ultimately succeeded (exit 0) with a normal Firefly cron response body — `restartPolicy: OnFailure` did exactly what it should. In the same ~4-minute window, `firefly` and `firefly-importer` (also pi-node3) independently failed their startup probes with the identical `connection refused`, and kubelet logged cgroup/containerd errors across *every* pod on the node (Wallabag, Vikunja, Traefik, ArgoCD components, node-exporter, promtail) — a node-wide event, not an app-level one.

Root cause: `journalctl -k` on pi-node3 showed repeated kernel `hwmon: Undervoltage detected!` messages (282 since the prior boot), `vcgencmd get_throttled` returned `0x50005` (undervoltage + throttling both live, not just historical), and core voltage measured as low as 0.86V. Rebooting the node did **not** fix it — `throttled` was back to `0x50005` within 2 minutes of the node coming back up, confirming this is a hardware power condition (PSU/cable/USB-SSD draw), not something software can clear. Every node in this cluster boots off a USB-attached SSD behind the same ASMedia bridge (see the `common` role's `uas` quirk fix), so all four boards share this exposure, not just pi-node3.

The kernel's own `node_hwmon_in_lcrit_alarm_volts` metric (already scraped by the existing node-exporter job) was checked live against `vcgencmd` during the incident and read non-alarm while `vcgencmd` reported a live undervoltage — it is not a reliable proxy for this condition on Raspberry Pi hardware, so it wasn't used for alerting.

**Fix (monitoring, not the hardware itself — that still needs a physical fix)**: `ansible/roles/common` deploys `rpi_throttle_metrics.sh` via cron (every minute) on all four nodes, polling `vcgencmd get_throttled` and writing a node-exporter textfile-collector metric (`node_rpi_throttled_bit`); `k8s/apps/node-exporter.yaml` mounts that directory and enables `--collector.textfile.directory`. Two new Prometheus alerts (`prometheus/alerts.yml`): `NodeUndervoltage` and `NodeThrottled`, both firing on the *live* bits (`_now`, not `_occurred`) so they reflect current state rather than a sticky since-boot flag. This makes the actual root cause visible directly instead of only showing up as second-order pod alerts on whatever happened to be running on the affected node at the time.

**Also found while investigating this**: `PodNotReady` had no exclusion for `Succeeded` phase, so it fired on `firefly-cron-29813880-ht6vx` for 3+ hours straight on an already-successful run and would never have auto-cleared — a completed Job/CronJob pod's `Ready` condition is permanently `False` (`reason: PodCompleted`) by design, not a real problem. Fixed by excluding pods currently in `Succeeded` phase (`kube_pod_status_phase{phase="Succeeded"}`) from the alert expression. Every CronJob on this platform (not just Firefly's) was generating a false alert on each successful run until this fix.

**Root cause of the undervoltage itself, established after this entry was first written**: not a per-node cable/board fault — all four nodes share a single UGREEN Nexode 200W 6-port GaN USB-C charger. GaN PD chargers are built to hit rated wattage at higher negotiated voltages (9/12/20V); the Pi 4 doesn't do PD negotiation and just draws fixed 5V, where these chargers' deliverable current is often well under what the wattage label implies, and this specific model's own spec shows per-port power dropping as more ports draw simultaneously (100W on one port, down to 65W/port with 3 active) — the exact trigger for a node-wide k3s event pulling current on multiple boards + their USB SSDs at once. Fix is hardware (official 5.1V/3A supply per board, or a plain non-PD 5V multi-port charger, plus getting the SSDs off each board's own power draw), not something this repo can patch.

**pi-node2 later hard-crashed and rebooted on its own** (undervoltage again, confirmed via `dmesg`), unprompted by any deliberate load — the power problem occurs during normal idle-ish operation, not only under the stress test added below. Confirms this isn't a "when convenient" fix.

**Follow-up bug, same night — the phantom-alert-restore issue recurred on a routine deploy, traced and fixed properly**: the `Jenkinsfile`'s Deploy stage unconditionally ran `docker compose up -d --force-recreate prometheus` on *every* deploy (necessary the one time `prometheus/alerts.yml`/`prometheus.yml` actually changes - they're bind-mounted as individual files, and a long-running container stays pinned to the inode it resolved at start, so `--force-recreate` is genuinely the only way to pick up an edited config; `docker exec prometheus cat alerts.yml` was confirmed showing 21-hour-stale content despite a fresh host write and a successful `-/reload` call). But that meant Prometheus restarted on *every* deploy, including ones that never touched `prometheus/` - and each restart re-triggered the stuck-firing-alert display bug (Prometheus's `ALERTS_FOR_STATE` restore-on-startup, default 1h outage-tolerance, doesn't reconcile against live truth for many minutes) using whatever alert history happened to be sitting in the persisted `prometheus-data` volume, real or already-resolved.

Two-part fix: (1) `docker-compose.yml` sets `--rules.alert.for-outage-tolerance=1m` on the prometheus service - caps how much stale "for" state a restart can ever restore, addressing the bug's actual mechanism regardless of what triggers a restart. (2) `Jenkinsfile`'s Checkout stage now diffs `prometheus/` between `GIT_PREVIOUS_SUCCESSFUL_COMMIT` and the current commit and only force-recreates prometheus when that diff is non-empty, so ordinary backend/frontend-only deploys leave it running (also keeps its metrics history intact instead of losing it every ~2 minutes on every poll cycle that ships a change).
