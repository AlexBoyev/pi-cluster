...
SSH access to pi-node1 is as `admin@10.100.102.10`. Both key-based (interactive/operator sessions) and password-based auth are enabled — the backend's own health-check SSH client (`SSHService`) authenticates with a password (`SSH_PASSWORD` in `.env`), so password auth is not disabled. Application code lives at `/home/admin/pi-cluster` (the path Jenkins actually deploys to — see `docs/decisions.md` for why this differs from Ansible's `platform_dir`). There is one operator identity across the whole cluster: `admin`/`admin` (password) on all 4 nodes, same for Ansible (`ansible_user: admin` in `ansible/group_vars/all.yml`), interactive SSH, and `SSHService` alike — an earlier version of this doc claimed a separate `alex` Ansible-only identity, which never existed in practice; corrected 2026-09-07, see `docs/decisions.md`.

**Known hardware quirk**: all 4 nodes' root filesystems are Kingston SA400S37 SSDs behind an identical ASMedia ASM235CM USB-SATA bridge (USB ID `174c:235c`). Linux's `uas` driver for that bridge is known to hang on the first read burst after a cold boot on Raspberry Pi — caused an 18+ minute full outage of every K8s-hosted service on 2026-09-07 before being root-caused and fixed with a kernel boot parameter (`usb-storage.quirks=174c:235c:u` in each node's `/boot/firmware/cmdline.txt`, applied via `ansible/roles/common`). Full incident and fix in `docs/decisions.md`.
...
- New-login-IP detection fires an unfiltered, never-muted alert to every notification channel on any login from an IP not previously seen for that user — see §24
- Real client IP is resolved via `Cf-Connecting-Ip` behind the Cloudflare Tunnel, falling back to `$remote_addr` for direct-LAN requests (nginx `map`, `nginx/nginx.conf`) — `$remote_addr` alone would only ever see `cloudflared`'s own local connection for tunnel traffic, defeating both rate limiting and new-login-IP detection for anyone accessing publicly
- The GitHub PAT used to publish alert-rule edits (§25) is a fine-grained token scoped to this one repository with Contents-write only, held in `.env` and never written to `admin@pi-node1`'s own git config
...
A separate category from everything above: self-hosted apps for household use (2 users) that happen to run on this cluster, not part of the platform itself — they don't manage nodes, workloads, or each other. Wallabag (read-later) is the first; Vikunja (shared task management) is the second; Paperless-ngx (document archive) is the third; Firefly III (personal finance) is the fourth and completes the originally-planned set. Full reasoning in `docs/decisions.md`'s Wallabag/Vikunja/Paperless/Firefly ADRs — this section is the pattern summary, kept current as more services land.

**Where Firefly reused the trusted-header pattern directly**: Firefly III's `remote_user_guard` authentication mode is architecturally identical to Paperless's `PAPERLESS_ENABLE_HTTP_REMOTE_USER` — same `X-Pi-User` header, same dedicated nginx block shape, no new mechanism needed. Its Data Importer companion (a separate upstream image, kept as its own Deployment) is instead gated by the standard wildcard SSO fallback, not the trusted-header block — it's an occasional admin tool authenticated by a one-time Personal Access Token, not something a per-user identity mechanism helps with. Full reasoning in `docs/decisions.md`'s Firefly ADR.

**Where Paperless needed the pattern extended, not just reused**: two more cases the Wallabag/Vikunja pattern hadn't hit yet.
- **A dedicated nginx block instead of the shared wildcard, for a service with its own trusted-header SSO support.** Paperless's `PAPERLESS_ENABLE_HTTP_REMOTE_USER` accepts pre-verified identity directly — no stored-credential bridge like Wallabag/Vikunja need. `GET /api/v1/auth/verify` returns the authenticated username in an `X-Pi-User` response header on success; a dedicated `paperless.*` nginx `server` block (not the household-services wildcard, which never reads this header) captures it via `auth_request_set` and forwards it as `Remote-User` — `proxy_set_header` always replaces the header outright, so a client-forged one can't get through. Reused directly for Firefly, above.
- **`hostNetwork: true` for a non-HTTP ingestion path.** Paperless's Samba consume-folder share (docs/decisions.md D2) needs literal port 445 — SMB clients (Windows Explorer especially) don't cleanly support the arbitrary high ports a K8s NodePort Service would assign. `hostNetwork: true` binds the pod directly to its node's real interface at the standard port instead, the same technique this cluster's own `dnsmasq`/`cloudflared` Compose services already use (`network_mode: host`) for the identical reason — reusable for any future non-HTTP household-service protocol.

**Where the pattern needed extending for Vikunja, not just reusing:** Vikunja is genuinely multi-user (assignment, permissions, shared projects, not two people sharing one login) and needs to reach phones off the LAN for CalDAV — two things Wallabag's first-draft pattern never had to account for. Two concrete additions came out of that, both reusable for Paperless/Firefly if they ever need the same:
- **Non-interactive endpoints must bypass the SSO gate entirely.** CalDAV clients (DAVx5, iOS) authenticate with plain HTTP Basic Auth and cannot follow a redirect to an HTML login page — the `pi_sso` gate below would silently break them exactly the way Cloudflare Access would. Vikunja's `/dav/*` is carved out of `auth_request` in nginx's wildcard block, relying on Vikunja's own per-user CalDAV app-password instead.
- **An auto-login bridge can't assume one shared credential** once a service has real distinct per-person accounts (see the SSO-gate note below) — it needs a small per-pi-cluster-user credential map instead of Wallabag's single shared one.

**The pattern, established once and reused:**

- **One namespace per service** (`wallabag`, `vikunja`, eventually `paperless`, `firefly`) — own `ResourceQuota`, clean uninstall, no cross-service Service-name collisions.
...
- **Node placement**: pi-node1 takes no household-service workloads (control plane + Docker Compose stack only). Each service is pinned to a specific worker by convention, not left to the scheduler — pi-node3 hosts Wallabag, Vikunja, and Firefly III together (combined footprint well under the node's ~7.64Gi allocatable, see `docs/decisions.md`), pi-node2 reserved for Paperless, pi-node4 doubles as the backup target.
...
**Ingress routing path**: `*.pi-cluster.lan`/`*.cluster.download` (dnsmasq wildcard) → nginx on pi-node1 → wildcard fallback `server` block (any hostname not matching a specific platform block) → `auth_request` SSO gate (below) → `upstream` of all three workers with passive health checks → Traefik (whichever worker answers) → K8s `Ingress` → Service → pod. See `nginx/nginx.conf` and the ADR for why this replaced an earlier per-hostname-block idea.

**SSO gate, not shared login**: household services keep their own independent user system (nothing here talks to the platform's Postgres `users` table), but you still can't reach one at all without an active pi-cluster session — nginx's wildcard block runs `auth_request` against `GET /api/v1/auth/verify` on every request, checking a `pi_sso` cookie set on platform login (`Domain=.pi-cluster.lan`, `HttpOnly`, its own long-lived token type distinct from the SPA's access/refresh tokens). No valid cookie → redirected to the platform login instead of ever reaching Traefik. Full reasoning (including why this lives in nginx and not a Traefik `forwardAuth` Middleware) in `docs/decisions.md`.

**Auto-login bridge, on top of the gate**: passing the SSO gate still lands on each service's own login screen — a per-service bridge closes that last gap for whichever pi-cluster user is asking. `WallabagBridgeService`/`VikunjaBridgeService` (`backend/app/services/`) log into the target service server-side using credentials held only in `.env`, and hand the resulting session to the browser via a two-hop redirect through the target service's own origin (e.g. `/api/v1/auth/vikunja-sso` → `vikunja.cluster.download/__vikunja_sso_finish`) so the final cookie can be set host-only, matching that service's native cookie scope exactly. Wallabag has one shared account, so the bridge always logs into it; Vikunja has two real distinct accounts, so its bridge looks up *this* pi-cluster user's own Vikunja login from a small credential map (`VIKUNJA_BRIDGE_CREDENTIALS` in `.env`) instead. Vikunja's finish step additionally has to inject the access-token JWT into the SPA's own `localStorage['token']` (a small same-origin HTML response, not a bare redirect) — its frontend only attempts a cookie-based silent refresh when it already finds an *expired* JWT there, so a plain redirect with only the refresh cookie set left a fresh browser session back on Vikunja's own login page. Full incident writeup in `docs/decisions.md` and `docs/roadmap.md` Phase 62.

## 24. Security Alerts

A dedicated, never-muted notification path distinct from the infra-alert pipeline in §9, for events where "someone may be compromising your account" — not "a CPU is hot."

**New-login-IP detection**: every successful `POST /api/v1/auth/login` checks the authenticating user's IP (`Cf-Connecting-Ip` behind the Cloudflare Tunnel, `$remote_addr` otherwise — see `docs/decisions.md`) against `known_login_ips` (migration 0012). An IP never seen before for that user fires `dispatch_security_alert` (`app/api/v1/auth.py`) to every enabled notification channel, **regardless of channel type or configured severity threshold** — there is no lower-severity version of this event the way there is for infra alerts, so it is never filtered.

**Delivery**: `NotificationChannel` (migration 0010, extended by 0012/0013) supports two `channel_type`s — `webhook` (original, unfiltered) and `email` (new, filtered — see below). Email delivery goes through a Brevo SMTP relay (`BREVO_SMTP_*` in `.env`) shared with Vikunja's own reminder mail (§23); distinct sender addresses (`alerts@`, `vikunja@`) for readability, same underlying relay and the same SPF/DKIM/DMARC domain-authentication requirement documented in the Vikunja ADR.

**Severity filtering — infra alerts only**: `dispatch_alert_notification` (the existing Prometheus/AlertManager firing pipeline, §9) sends every firing unchanged to `webhook` channels, but for `email` channels only sends alerts at or above that channel's own `min_severity` (`warning` or `critical`, configurable per-channel in the Notifications UI — migration 0013). This was an explicit fix after the alert system first shipped: routine infra noise (HighCPU, HighMemory, HighDisk, HighTemperature, PodCrashLooping, PodNotReady — all `warning`) was reaching the same inbox as real incidents and defeating the point of email as an urgent-only channel. `dispatch_security_alert` above is never subject to this filter. Full rationale in `docs/decisions.md`.

## 25. Alert Rules Management (admin panel)

Prometheus alert rules (`prometheus/alerts.yml`, §9) can be added, edited, and deleted from the dashboard's Alert Rules page instead of hand-editing the file over SSH.

`AlertRulesService` (`backend/app/services/alert_rules_service.py`) reads and writes `alerts.yml` on pi-node1 over SSH, using `ruamel.yaml`'s round-trip loader (preserves comments, key order, and formatting — a plain `yaml.safe_load`/`dump` round trip would strip every comment in the file on the first UI edit). Every read and write first re-syncs the local clone to `origin/master` (`git fetch && git reset --hard`) rather than trusting whatever the clone happened to have, since it's normally kept current by Jenkins' rsync, not git. A change is committed and pushed with a fine-grained, Contents-write-only GitHub PAT (`GITHUB_PAT` in `.env`, scoped to this one repo) passed as a one-off push target rather than stored in `admin@pi-node1`'s own git config — so the token never appears in `git remote -v`. After a successful push, Prometheus is recreated (`docker compose up -d --force-recreate`, not `-/reload`) to pick up the change — `alerts.yml` is a single-file bind mount pinned to the inode it resolved at container start, so a file replacement (git checkout, not an in-place edit) needs a real container recreation to be picked up, the same latent bug `Jenkinsfile`'s own Deploy stage had via rsync.

`POST/PATCH/DELETE /api/v1/prometheus/rules` are admin-only, same as the router's existing `GET`. A validation failure (duplicate rule name on create, not-found on update/delete) maps to `409`/`404` without ever touching SSH; a failure partway through the publish pipeline (permissions, git sync, the write itself, or the Prometheus recreate) maps to `502` with the real failed step named — the original version of this code logged SSH command output and returned success regardless of it, silently swallowing both a permission-denied write and a non-fast-forward-rejected push.
