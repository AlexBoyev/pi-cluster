...
A separate category from everything above: self-hosted apps for household use (2 users) that happen to run on this cluster, not part of the platform itself — they don't manage nodes, workloads, or each other. Wallabag (read-later) is the first; Vikunja (shared task management) is the second; Paperless-ngx and Firefly III are planned to follow the same pattern. Full reasoning in `docs/decisions.md`'s Wallabag and Vikunja ADRs — this section is the pattern summary, kept current as more services land.
