...
- **Node placement**: pi-node1 takes no household-service workloads (control plane + Docker Compose stack only). Each service is pinned to a specific worker by convention, not left to the scheduler — pi-node3 hosts both Wallabag and Vikunja (combined footprint well under the node's ~7.64Gi allocatable, see `docs/decisions.md`), pi-node2 reserved for Paperless, pi-node4 doubles as the backup target.
