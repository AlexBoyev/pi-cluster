...
- [x] **Fix actually deployed and verified live** — discovered `k8s/traefik/traefik.yaml` isn't GitOps-managed (ArgoCD only watches `k8s/apps/`, confirmed via `kubectl get application ... -o jsonpath`), so pushing the fix to git did nothing by itself; applied by hand (`kubectl apply -f k8s/traefik/traefik.yaml`), deleted the stuck pi-node1 pod, and confirmed the DaemonSet settled at 3/3 (pi-node2/3/4 only), all with 0 restarts post-fix
- [ ] Cluster-wide restart pattern not fully root-caused — all 4 pods had high restart counts (8-12) before this fix, and during the rollout itself two *different* pods (pi-node1's old one, then pi-node3's) got stuck `Terminating` past their grace period and needed `--grace-period=0 --force`, including on nodes with no port conflict. The pi-node1 port conflict is fixed and confirmed; a second, unidentified Traefik/K3s termination issue may still exist — watch the new `PodCrashLooping`/`PodNotReady` alerts rather than considering this fully closed
- [ ] `k8s/traefik/traefik.yaml` still isn't GitOps-managed — moving it into `k8s/apps/` (so future changes don't require a manual `kubectl apply`) is a worthwhile follow-up, not done here
...
## Phase 58 — Wallabag (first household service) ✓
- [x] ADR written first (`docs/decisions.md`, D1-D4) and iterated with real pushback before any manifest existed — storage (`local-path` + `nodeSelector`, not NFS — no NFS infra exists on this cluster), database (new `wallabag` db + dedicated Postgres role inside the existing platform Postgres, not a new pod, not SQLite), namespace (`wallabag`, dedicated, the convention for the next 3 services), ingress (see below)
- [x] Discovered and fixed a real, previously-invisible gap while designing D3: there was no working path from any hostname to Traefik on pi-node2/3/4 at all (Traefik was excluded from pi-node1 in an earlier phase; nginx there only proxies a fixed set of platform hostnames). Built a wildcard nginx fallback (`*.pi-cluster.lan`/`*.cluster.download` → `upstream` of all 3 workers with passive health checks) instead of a per-service nginx edit — this is infrastructure the other 3 planned services need too, built once
- [x] `ansible/roles/backup` widened from a single hardcoded database to a list (`backup_postgres_databases: [pi_cluster, wallabag]`), looped in the template — verified the rendered output before relying on it
- [x] Manifests in `k8s/apps/wallabag/`: Namespace, PVC (`local-path`, 5Gi), ConfigMap, Deployment (pinned `pi-node3`, `Recreate` strategy since local-path is RWO), Service, Ingress (`ingressClassName: traefik`, LAN-only host) — no Secret manifest in git, by design (would fight ArgoCD's `selfHeal`); creation documented as a manual step instead
- [x] Image `wallabag/wallabag:2.6.14` verified for `linux/arm64` via a direct Docker Hub registry API query, not a docs page summary — pinned, never `latest`
- [x] `docs/operations.md` (new) — drain-impact table (Wallabag is the first service that actually dies when its node is drained), manual node-migration procedure written now while still theoretical, deploy/upgrade/rollback/add-a-user runbooks
- [x] `docs/architecture.md` §23 — the household-services pattern, written to generalize to Vikunja/Paperless/Firefly, not just describe Wallabag
- [x] No AI/enrichment in this phase — `.env.example` gets placeholder `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` (unused, reserved for a future companion service polling Wallabag's API — never built into the container itself)
- [x] **Applied to the live cluster and running** — Postgres role/database, Secret, and all manifests deployed; pod `Running`, `1/1 Ready`, probes passing.
- [x] `DOMAIN_NAME` verified against the real running instance — absolute URLs in the actual served HTML correctly read `http://wallabag.pi-cluster.lan`. Browser-extension/mobile-app not tested (needs a physical device on the LAN), but the URL they'd be configured against is confirmed correct.
- [x] Full ingress path verified end-to-end with real HTTP requests (nginx wildcard → upstream → Traefik → Ingress → Service → pod): `curl -H 'Host: wallabag.pi-cluster.lan' .../login` → `200`, real Wallabag login page.
- [x] Registration verified closed: `/register` redirects to `/login`, both accounts created via CLI (`bin/console fos:user:create`) instead — see corrected README.md.
- [x] Logs confirmed in Loki with real content: `{namespace="wallabag"}` query returns actual request lines with correct `pod`/`container`/`node_name` labels — not just "the pod exists," the log pipeline was queried directly and inspected.
- [ ] `wallabag:install` had to be run manually once (not automatic, despite `POPULATE_DATABASE=True`) — see the corrected D2 configuration notes in `docs/decisions.md` and the corrected `k8s/apps/wallabag/README.md`. A genuine extra step, not yet automated away.

**Three infrastructure bugs found and fixed while actually deploying this** (full detail in `docs/decisions.md`) — none Wallabag-specific, all would have silently blocked Vikunja/Paperless/Firefly too, and two were already broken cluster-wide with no prior workload having exercised them end-to-end:
- Traefik's ClusterRole was missing `discovery.k8s.io/endpointslices` and `nodes` — every Ingress-routed request 404'd, cluster-wide, before this fix. **No Ingress-based workload had ever actually worked on this cluster before now.**
- Promtail's Kubernetes pod-log scraping matched zero targets, cluster-wide, since the file was first written — a too-shallow `__path__` glob plus a self-node-selector bug (`HOSTNAME` env var does nothing for Go's `os.Hostname()`; needed `hostNetwork: true` instead, same reason node-exporter already has it). Also bumped `grafana/promtail` 2.9.4 → 3.6.11 while investigating (turned out not to be the actual cause, kept as a reasonable modernization regardless).
- Both bugs were **completely invisible to `kubectl get pods`** — every pod showed `Running`/`Ready`/`0 restarts` the whole time. Found only by checking actual behavior (RBAC via `kubectl auth can-i`, reading logs directly, querying Loki's real content) rather than trusting green status — worth remembering for Vikunja/Paperless/Firefly: pod health proves nothing about whether ingress or logging actually work.

## Phase 59 — Household Services SSO Gate ✓
- [x] `pi_sso` cookie (`Domain=.pi-cluster.lan`, `HttpOnly`, dedicated long-lived `sso`-typed JWT distinct from the SPA's access/refresh tokens) set on platform login, alongside the existing `localStorage` JWT flow
- [x] `GET /api/v1/auth/verify` — validates the cookie; nginx's wildcard household-services `server` block calls it via `auth_request` on every request, bouncing to the platform login (`error_page 401 = @pi_sso_login`) if missing/invalid — gates *reachability*, not identity inside each app, which keeps its own independent login
- [x] Dashboard gained a **Household Services** tile section (Wallabag, later Vikunja) with an `ssoBridge` flag per tile
- [x] Three real bugs found wiring this up live: the cookie was scoped to a domain nobody actually logs in on (fixed the `Domain` value), the Wallabag Ingress only matched `wallabag.pi-cluster.lan` and 404'd on the public `.cluster.download` host (added the second `rules[]` entry — Traefik matches `Host` by exact string, no `.lan`/`.cluster.download` equivalence), and the dashboard's own Wallabag link was hardcoded to the `.lan` host, breaking the gate for anyone using the public domain
- [x] Rejected a Traefik `forwardAuth` Middleware for this gate — the CRDs it needs were never installed on this cluster's hand-deployed Traefik, and nginx already terminates every household-service request first with `auth_request` built in (full reasoning: `docs/decisions.md`)
- [x] **Mobile-only bug found live** (Android and iPhone, never reproduced on desktop): the household-service tiles used `target="_blank"`, opening a fresh tab and racing it through the multi-hop SSO redirect chain (gate → bridge → target app) — mobile Safari/Chrome silently kill a newly opened tab that never paints before redirecting cross-origin several times, which showed up as the tile flashing open then bouncing straight back to the dashboard. Fixed by switching all three tile render sites to same-tab navigation (`frontend/src/App.tsx`) — no popup-tab path left to trip the heuristic on any platform

## Phase 60 — Wallabag Auto-Login Bridge ✓
- [x] `WallabagBridgeService` + `/api/v1/auth/wallabag-sso` — logs into Wallabag's one shared account server-side on behalf of any pi-cluster user with a valid `pi_sso` cookie, so the household tile skips Wallabag's own login screen entirely
- [x] Two real bugs found live: the bridge silently swallowed its own failure reason (fixed to surface a real error instead of a same-looking blank failure), and the bridge's session cookie collided with any pre-existing native Wallabag session in the same browser (Domain-scoped collision — same class of bug Vikunja's bridge hit later via Path instead of Domain, see Phase 62)

## Phase 61 — Vikunja (second household service) ✓
- [x] ADR written first (`docs/decisions.md`) — ADR covers public exposure (app-level auth only, no Cloudflare Access — CalDAV's Basic Auth can't follow an interactive login redirect), Brevo SMTP for reminder mail, CalDAV app-password auth, a smaller 2Gi attachments PVC, and a shared "Household" team instead of per-project shares
- [x] `/dav/*` carved out of the `pi_sso` gate entirely (`location ~ ^/dav/` in `nginx/nginx.conf`, no `auth_request`) — the first case the Wallabag-era gate design didn't anticipate, since Wallabag has no non-interactive client
- [x] Manifests in `k8s/apps/vikunja/` mirroring Wallabag's pattern (namespace, `local-path` PVC, ConfigMap, Deployment pinned to pi-node3, Service, dual-host Ingress) — single combined `vikunja/vikunja:2.6.0` image, migrations run automatically on every start unlike Wallabag
- [x] Registration disabled, verified closed from outside the LAN; two real accounts created via CLI, both added to a shared "Household" team
- [x] Fixed `K8sService.list_ingresses()` crashing on any ingress with `ingressClassName` set — found deploying Vikunja's Ingress, not Vikunja-specific, would have broken the Services & Ingresses page for any future ingress using the field
- [x] Fixed: household services were invisible to viewer-role users (dashboard tile visibility bug, found immediately after Vikunja shipped a second tile to look at)
- [x] Full ingress path and CalDAV `/dav/` exemption verified live with real HTTP requests, both hostnames (see `docs/decisions.md` for the exact verification requests) — plus a real, unrelated Promtail regression found and fixed while checking Vikunja's logs reached Loki (all 4 DaemonSet pods stuck post a cluster restart, invisible to `kubectl get pods` since Promtail has no liveness/readiness probes)

## Phase 62 — Vikunja Auto-Login Bridge ✓
- [x] `VikunjaBridgeService` + `/api/v1/auth/vikunja-sso` — unlike Wallabag's one shared account, Vikunja has two real distinct accounts, so this looks up *this* pi-cluster user's own Vikunja login from a small per-user credential map (`VIKUNJA_BRIDGE_CREDENTIALS` in `.env`) instead of one shared credential
- [x] Two-hop handoff (`vikunja-sso` → `vikunja-sso-finish`, reached via redirect through `vikunja.cluster.download` itself) so the final cookie can be set host-only, matching Vikunja's own native cookie scope exactly (`Path=/api/v1/user/token/refresh` and `/api/v2/...`) — fixes a reproduced Path-based cookie collision with any pre-existing native Vikunja login session, the same collision class Wallabag hit via `Domain` in Phase 60
- [x] **Real bug found live in a fresh-incognito browser test, invisible to every curl-based check**: a bare redirect to `/` after setting the refresh-token cookie left the user back on Vikunja's own `/login` — confirmed via Vikunja's own access logs that a real browser only ever called `GET /login`, never `POST /api/v1/user/token/refresh`, because Vikunja's frontend (`checkAuth()`) only attempts a cookie-based silent refresh when it already finds an *expired* JWT sitting in `localStorage['token']`; on a genuinely fresh load there's nothing to find, so it never touches the refresh cookie at all. Fixed by capturing the access-token JWT already returned in Vikunja's own login response body (previously discarded) and having the finish step write it into `localStorage['token']` via a small same-origin HTML page before handing off to the SPA, instead of a bare redirect

## Phase 63 — Security Alerts: New-Login-IP Detection + Brevo Email ✓
- [x] Migration 0012 — `known_login_ips` table (per-user seen-IP history) and `notification_channels` gains `channel_type` (`webhook`/`email`) + `email_address`
- [x] New-login-IP detection on every successful `/auth/login` — an IP not previously seen for that user triggers `dispatch_security_alert`, which is **never** severity-filtered, for either channel type (explicit: there's no lower-severity version of "someone's hacking my account" the way there is for infra alerts)
- [x] Brevo SMTP relay wired as the platform's outbound mailer (`BREVO_SMTP_*` in `.env`) — same relay account Vikunja's reminder mail uses (see Phase 61's ADR), distinct sender addresses (`alerts@`, `vikunja@`) for readability
- [x] Explicit user request after the alert system first shipped: routine infra alerts (HighCPU, HighMemory, HighDisk, HighTemperature, PodCrashLooping, PodNotReady — all `warning`) were reaching the same inbox as real security events and defeating the point of email as an urgent-only channel. Fixed: `dispatch_alert_notification` only sends `email`-type channels alerts at or above their `min_severity` (webhook channels stay unfiltered — a webhook like Slack tolerates volume fine and can be muted per-channel there)
- [x] Migration 0013 — `notification_channels.min_severity` (`warning`/`critical`, default `warning`), made configurable per-channel from the Notifications UI instead of hardcoded, per explicit follow-up request ("is that configurable in the UI? lets add it")
- [x] `NotificationsPage.tsx` gained channel-type and per-channel severity-threshold selectors

## Phase 64 — Alert Rules CRUD (admin panel) ✓
- [x] `POST/PATCH/DELETE /api/v1/prometheus/rules` (admin-only, matching the router's existing `GET`) — add, edit, and delete Prometheus alert rules directly from `AlertRulesPage.tsx` instead of hand-editing `prometheus/alerts.yml` over SSH
- [x] `AlertRulesService` reads/edits/writes `prometheus/alerts.yml` on pi-node1 over SSH using `ruamel.yaml`'s round-trip loader (preserves comments and formatting — a plain `yaml.safe_load`/`dump` round trip was confirmed live to strip every comment in the file on the first edit) and publishes via a one-off `git push` using a fine-grained GitHub PAT (`GITHUB_PAT` in `.env`, Contents-write-only, scoped to this repo) rather than storing push credentials in `admin@pi-node1`'s own git config
- [x] **Three real, compounding bugs found live, not just written**: `/home/admin/pi-cluster` was root-owned from Jenkins' rsync, so a "successful" write silently never landed (fixed: `chown` + exit-code-checked SSH commands instead of logging output and returning success regardless); the local clone was frozen months behind `origin/master` since it was only ever kept current by rsync, never `git pull` — so pushes were silently rejected as non-fast-forward (fixed: `git fetch && git reset --hard origin/master` before every read and write); and `prometheus/alerts.yml` is a single-file Docker bind mount pinned to the inode it resolved at container start, so a `-/reload` call after a `git checkout`-style file replacement kept serving 21-hour-stale content regardless of how many times it was called (fixed: `docker compose up -d --force-recreate prometheus` — the same latent bug existed in `Jenkinsfile`'s own Deploy stage via rsync and was fixed there too)

## Phase 65 — Paperless-ngx (third household service) ✓
- [x] ADR written and confirmed before any manifest existed (`docs/decisions.md`) — Tika+Gotenberg for Office document support, a dedicated Samba share as the primary authenticated ingestion path (not NFS, not Syncthing, not web-upload-only), no invented search-reindex benchmark, and `PAPERLESS_ENABLE_HTTP_REMOTE_USER` instead of a stored-credential bridge — a genuine reduction of the "credential doubling" tradeoff Wallabag/Vikunja both accepted, not a repeat of it
- [x] Manifests in `k8s/apps/paperless/` — namespace, 20Gi `local-path` PVC (`media`/`data`/`consume`/`export` subPaths, one-PVC convention matching Wallabag/Vikunja), Deployment pinned to pi-node2 **alone** (not alongside Wallabag/Vikunja on pi-node3 — OCR pins a CPU core), Tika + Gotenberg as separate in-namespace Deployments, a `hostNetwork: true` Samba Deployment for real port 445. All four images verified `linux/arm64` via direct registry manifest queries, pinned, never `latest`
- [x] Backend: `GET /api/v1/auth/verify` now returns the authenticated username in an `X-Pi-User` response header on success; a new **dedicated** nginx `paperless.*` server block (not the shared household-services wildcard) captures it via `auth_request_set` and forwards it as `Remote-User` — the first service using this SSO mechanism instead of a bridge
- [x] **Three real bugs found deploying this live, not just written**: Tika's JVM startup is too slow on Pi-class ARM for a bare liveness probe with no `startupProbe` — it killed the container in a restart loop before the JVM ever bound its port, fixed by adding one (same pattern the main container already had). Naming the K8s Service "paperless" collided with Kubernetes' own legacy `<ServiceName>_PORT` env-var auto-injection, producing a `PAPERLESS_PORT` value that crashed paperless-ngx's own ASGI server trying to parse a Kubernetes service URI as an integer port — fixed with `enableServiceLinks: false` on every Deployment in the namespace. A real, previously-reported upstream issue (`paperless-ngx#3214`) meant kubelet's probes connecting via the pod's real IP got rejected with `DisallowedHost` — fixed by forcing the probe's `Host` header to `localhost`, which the image's own Dockerfile healthcheck already relies on being always-allowed, instead of touching `ALLOWED_HOSTS`
- [x] **Applied to the live cluster and verified end-to-end, not just "pod is Running"**: all four pods (paperless, tika, gotenberg, samba) confirmed `1/1 Running`; migrations applied and superuser created from the Secret (confirmed in the pod's own log); the full SSO chain verified for real — `pi_sso` cookie → nginx `auth_request` → `/verify`'s `X-Pi-User` header → `Remote-User` → Paperless's `RemoteUserBackend` → a genuine Django session, proven by using *only* the resulting `sessionid` cookie against `/api/documents/` and getting `200`, not just a plausible-looking page load. Confirmed separately that a request with no `pi_sso` cookie still gets bounced to the platform login (`302`), and that `/api/documents/` still returns `401` without a real session — the API's isolation from the web SSO gate holds
- [x] `ansible/roles/backup`'s database list gained `paperless` — applied directly to the live rendered `backup.sh` on pi-node1 (no local Ansible available this session) rather than left as a doc-only change, since the nightly cron's `set -euo pipefail` would have aborted the whole run (including the K3s datastore snapshot) against a database that didn't exist yet. The Postgres role/database itself was also created immediately for the same reason, not deferred to the README's manual-steps list
- [x] Recognized the media-volume capacity alert the brief asked for is **already covered** by the existing `HighDisk` rule (`prometheus/alerts.yml`) — `local-path` PVCs have no real per-volume quota, so the number that matters is pi-node2's actual disk usage, which that rule already watches at 80% for any node. Documented explicitly rather than adding a redundant near-duplicate rule
- [ ] **Not yet done, needs the user**: adding the Cloudflare Tunnel Public Hostname route (`paperless.cluster.download` → `http://10.100.102.10:80`, same manual step as Wallabag/Vikunja); a real Hebrew-and-English document ingested and its OCR text read by eye; Hebrew full-text search confirmed to return the correct document; a real Samba client connection tested from an actual device; a real timed `document_index reindex` run recorded in `docs/operations.md`

## Phase 66 — Cold-boot outage: USB `uas` driver fix + Traefik health probes ✓

- [x] **Incident, 2026-09-07**: a full power cycle (all 4 Pis share one power source, no way to stagger physically) caused an 18+ minute total outage of every K8s-hosted service (Wallabag, Vikunja, Paperless, the dashboard itself). First hypothesis — cross-node cold-boot I/O contention — led to a per-node `k3s-agent` boot-delay stagger (`ansible/roles/k3s_agent`, `boot_delay_seconds` in `ansible/inventory/hosts.ini`); real root cause, found after that fix didn't actually resolve a reproduction of the same incident, was the `uas` USB driver hanging on the first read burst after cold boot for the ASMedia bridge (`174c:235c`) behind all 4 nodes' Kingston SSDs — fixed with `usb-storage.quirks=174c:235c:u` in `/boot/firmware/cmdline.txt` on all 4 nodes (`ansible/roles/common`). Full incident, the wrong-then-right diagnosis, and verification detail in `docs/decisions.md`
- [x] `k8s/traefik/traefik.yaml` gained `startupProbe`/`readinessProbe`/`livenessProbe` against Traefik's `/ping` endpoint — previously no probes existed at all, so `kubectl get pods` reported a completely wedged Traefik pod as `1/1 Running` for the full 18+ minutes, making the outage invisible to the platform's own tooling and forcing manual `crictl`/`ps`/`nsenter` diagnosis on each node
- [x] Fixed a real, silent bug found while applying the boot-delay fix: `ansible/roles/common`'s existing cgroups task had been targeting `/boot/cmdline.txt`, a placeholder file on this Raspberry Pi OS version ("DO NOT EDIT THIS FILE... moved to /boot/firmware/cmdline.txt") — it had silently no-op'd since some earlier OS upgrade. Corrected to the real path
- [x] Corrected two stale doc claims found while fixing this: `docs/architecture.md` and `docs/decisions.md` both described a separate `alex` Ansible-only SSH identity that never existed in practice — `ansible/group_vars/all.yml` has always used `ansible_user: admin`, verified by reading the file directly
- [x] Tooling: `~/.ssh/config` with `BatchMode yes` for all 4 nodes (Git for Windows' `ssh` was popping GUI password prompts during background SSH calls); `plink.exe` with `-pw admin` established as the reliable fallback for pi-node3/4, where key-based auth was flaky at the time (root-caused and fixed below)
- [x] **Also explains Phase 56's unresolved mystery**: the "second, unidentified Traefik/K3s termination issue" flagged there (pods stuck `Terminating` past grace period on nodes with no port conflict) is the same `D`-state/`uas` driver issue — a genuinely unkillable kernel disk-wait, not a Traefik or K3s bug at all
- [x] **Follow-up closed same day**: `k8s/traefik/traefik.yaml` moved into `k8s/apps/traefik.yaml` — ArgoCD-managed now, the manual-`kubectl apply` special case from Phase 8/56 is gone
- [x] **`dnsmasq` "restart loop" corrected — it was a misdiagnosis, not a bug.** Re-checked with real timestamps (`docker logs -t`): the "SIGTERM → started" pairs are one per day (Sept 1/2/4/5), each a normal `restart: unless-stopped` cycle tied to an actual host reboot, not rapid flapping — an untimestamped `docker logs --tail 30` during the live incident made several days' worth of routine restarts look like a tight loop. `RestartCount=0`, current instance stable 2+ days. Raspberry Pi's lack of a battery-backed RTC explains why the boot record itself looked stale (utmp/Docker's `StartedAt` get written before NTP corrects the clock)
- [x] **Key auth to pi-node3/pi-node4 root-caused and fixed**: pi-node3 had no `~/.ssh` directory at all (key trust never set up); pi-node4's `authorized_keys` had only an unrelated Ansible-generated key from the backup role (Phase 51), never the operator's own key. Both fixed by adding the real key to both nodes' `authorized_keys`; key auth confirmed working on all 4 nodes

## Phase 67 — Firefly III (fourth household service, completes the planned set) ✓

- [x] ADR written first (`docs/decisions.md`) — CSV/OFX import via the official Data Importer companion (bank-API auto-sync explicitly flagged as unverified for Israeli banks, no PSD2 basis to expect coverage), `remote_user_guard` trusted-header SSO reusing Paperless's exact mechanism instead of a new stored-credential bridge, joins pi-node3 alongside Wallabag/Vikunja, a K8s `CronJob` for the daily recurring-transactions endpoint instead of a hand-rolled cron sidecar
- [x] Manifests in `k8s/apps/firefly/`: namespace, PVC, ConfigMap, Deployment (core + a second Deployment for the Data Importer, each its own Service/Ingress), `cronjob.yaml`. No Secret manifest in git, by design
- [x] **Applied to the live cluster and verified end-to-end**: Postgres role/database created, Secret created (Brevo mail credentials reused from the existing Vikunja relay account, distinct sender address), both pods `1/1 Ready`, real HTTP requests through the full path (nginx → Traefik → Ingress → Service → pod) return `302` (SSO redirect, correct unauthenticated) for both `firefly.cluster.download` and `firefly-import.cluster.download`
- [x] **Real bug found and fixed live**: the `startupProbe` budget (150s) was too short for Firefly's genuine first-boot `composer dump-autoload` + migration time on pi-node3's ARM hardware — caused an actual restart-before-ready loop on first deploy. Widened to 600s (`k8s/apps/firefly/deployment.yaml`), same class of mistake this project already paid for once with Paperless/Tika's slow JVM boot
- [x] **Much bigger bug found while diagnosing why Firefly's manifests weren't syncing at all**: the live ArgoCD Application had no `directory.recurse` set, so it had only ever applied root-level files in `k8s/apps/` — Wallabag, Vikunja, and Paperless had silently never been GitOps-managed since each was first deployed, only ever reachable via each README's manual `kubectl apply -f k8s/apps/<name>/` fallback. Also discovered neither `terraform/argocd.tf` (wrong Application name/namespace) nor `ansible/playbooks/argocd.yml` (only ever a one-time `kubectl apply`, no watching resource) matched the real live Application at all — its actual origin is unknown. Patched the live resource directly and corrected `terraform/argocd.tf`; verified Wallabag/Vikunja/Paperless pods were undisturbed (same names/ages, zero new restarts) and Firefly's manifests were picked up within seconds of the fix. Full writeup in `docs/decisions.md`
- [x] Jenkins build #146 failed after this landed: the new `backend/tests/test_docs_consistency.py` (Phase 66) crashed with `FileNotFoundError` — Jenkins's Test stage runs pytest *inside the built backend Docker image*, whose build context is `backend/` alone, so `docs/` and `frontend/` never exist in that container. Fixed to skip gracefully when those sibling paths aren't present rather than assume a full checkout; still runs for real on a local `pytest`
- [x] **Frontend tile was missing entirely** — the dashboard's `HOUSEHOLD_SERVICES` array (`frontend/src/App.tsx`) drives the Household Services tile grid, and despite its own comment saying "add an entry here as each one lands... Firefly III," no entry was ever added. Firefly was fully deployed and reachable by direct URL for a period with no dashboard link to it at all. Added the tile (`ssoBridge: false`, same as Paperless — trusted-header, no bridge needed)
- [ ] **Not yet done, needs the user**: Cloudflare Tunnel Public Hostname routes for `firefly.cluster.download` and `firefly-import.cluster.download`; the first admin account through Firefly's setup wizard; a real CSV import from an actual bank statement, run through the Data Importer and verified against the source data
