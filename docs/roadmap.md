...
- [ ] **Not yet applied to the live cluster** — needs the manual steps in `k8s/apps/wallabag/README.md` (Postgres role creation, Secret creation) before ArgoCD sync does anything useful
- [ ] `DOMAIN_NAME`/links/browser-extension/mobile-app not yet verified against a real running instance — can't be, until the manual steps above happen
- [ ] Logs-in-Loki, probes-passing, and registration-actually-disabled are all designed-for but unverified live for the same reason
