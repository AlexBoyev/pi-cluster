...
- [x] Tooling: `~/.ssh/config` with `BatchMode yes` for all 4 nodes (Git for Windows' `ssh` was popping GUI password prompts during background SSH calls); `plink.exe` with `-pw admin` established as the reliable fallback for pi-node3/4, where key-based auth is inexplicably flaky (not yet root-caused)
- [ ] `dnsmasq` on pi-node1 was observed restart-looping during the incident (SIGTERM every few seconds) — caused intermittent DNS/`sudo` failures while diagnosing, flagged but not fixed; separate open item
- [ ] Key auth to pi-node3/pi-node4 failing while pi-node1/pi-node2 work fine, same key — not root-caused, `plink`+password is the working fallback
